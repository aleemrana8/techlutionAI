...
export async function generateWeeklyReport(): Promise<Buffer> {
  ...
  return new Promise((resolve, reject) => {
    ...
    const summaryData = [
      ...
      ['Leads', totalLeads.toString(), weekLeads.toString()],
      ['New Leads (Pending)', newLeads.toString(), '-'],
      ['Converted Leads', convertedLeads.toString(), `${conversionRate}% rate`],
    ...
    if (recentLeads.length > 0) {
      doc.fontSize(16).fill(dark).text('Recent Leads')
      doc.moveDown(0.5)
...
export async function emailWeeklyReport() {
  try {
    ...
    await sendMail({
      ...
      html: `<div style="font-family:Arial,sans-serif;color:#e2e8f0;background:#070b14;padding:40px;">
        <h1 style="color:#f97316;">Techlution AI — Weekly Report</h1>
        <p style="color:#94a3b8;">Please find attached the weekly business report.</p>
